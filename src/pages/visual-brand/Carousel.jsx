...
// `full` (optional): full-width slides (one per view) instead of natural width.
export default function Carousel({ images, strike, full }) {
    ...
    <div className="tirl-wrap">
      ...
      >
        <div className="tirl__track" ref={trackRef}>
          {SLIDES.map((s, i) => (
            <div className={"tirl__slide" + (images ? (full ? " tirl__slide--full" : " tirl__slide--img") : "")} key={i}>
              {images ? <img className={"tirl__img" + (full ? " tirl__img--full" : "")} src={s.src} alt={s.alt || ""} draggable="false" /> : null}
              {images && strike ? (
